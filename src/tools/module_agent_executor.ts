...
6. **完成后直接结束会话**，结束前按以下顺序执行：
   - 若未写入执行总结，必须先执行 module_agent_updater(action="write_result", status="success|partial|failed", summary="执行总结")
   - 必须执行 module_agent_plan(action="plan_complete", files=["相对路径1", "相对路径2", ...]) 写入本次修改的文件列表
...
async function handleStatus(
  ...
  if (allRecords.length > 0) {
    ...
    return {
      title: `模块 '${module_name}' 执行${active ? '中' : '完成'}`,
      output: JSON.stringify({
        type: 'limu',
        finished: !active,
        records: allRecords,
        ...(active ? { status: active.status, current_work: active.summary } : {}),
        last_activity: activity ?? null,
        idle_seconds: idleSeconds,
        unresponsive: active ? unresponsive : false,
      }),
    }
  }
