...
import { readAgentModelConfig, writeAgentModelConfig, validateModelConfig } from '../lib/agent_model_config.ts'
        ...
        if (action === 'set') {
          ...
          if (lizhuProviderId || lizhuModelId) {
            if (!lizhuProviderId || !lizhuModelId) {
              return {
                title: '参数错误',
                output: JSON.stringify({ status: 'error', error: '设置离朱模型需同时提供 lizhu_provider_id 和 lizhu_model_id' }),
              }
            }
          ...
          const validationErrors = await validateModelConfig(client, candidate)
          if (validationErrors.length > 0) {
            return {
              title: '模型校验失败',
              output: JSON.stringify({ status: 'error', errors: validationErrors }),
            }
          }
          ...
          return {
            title: '模型配置已更新',
            output: JSON.stringify({ status: 'ok', config: candidate }),
          }
        }
