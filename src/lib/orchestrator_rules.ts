...
1. **工作空间初始化**：
   ...
   - 若无工作空间 → 询问用户："请输入新建工作空间名称（仅支持英文、数字、下划线）：" → 用户输入名称后 workspace(action="create", name="xxx")
     → 创建后必须执行以下模型配置步骤：
       ...
       b. 引导用户选择力牧和皋陶的默认模型，调用 agent_model_config(action="set", limu_provider_id="...", limu_model_id="...", gaotao_provider_id="...", gaotao_model_id="...")
   - 若有工作空间 → 展示列表给用户 → 询问用户："选择已有工作空间请输入名称，新建请输入新的工作空间名称："
     - 用户输入名称匹配已有空间 → workspace(action="bind", workspace_name="xxx")
       → 绑定后必须调用 agent_model_config(action="get") 检测当前工作空间是否已配置力牧和皋陶默认模型
       → 若未配置（config 为 null 或缺少 limu/gaotao）：调用 agent_model_list 展示可用模型，引导用户通过 agent_model_config(action="set", ...) 设置缺失的模型
