...
5. **询问用户**（必经步骤，不可跳过）：
    - 全部模块确认并启动完成后，生成确认码并告知用户："是否查询子任务状态并汇总执行结果？回复以下确认码查询子任务状态并汇总执行结果：[确认码]"
    - 若用户输入正确确认码 → 进入「子任务状态检查和汇总调度」。
...
1. **统一轮询状态**：
    ...
    - 待所有力牧和皋陶全部 finished=true 后，进入步骤 2

2. **汇总报告**：向用户汇报所有模块的执行结果和审查结果（如有），提醒用户通过 module_agent_plan(action="clean_completed") 清理已审查的计划，再通过 module_agent_done 关闭已完成任务的力牧和皋陶。
...
- 若全部未 finished，告知用户"力牧还在执行，请等候"。注意：此处仅做一次性查询，不要轮询。轮询仅在「子任务状态检查和汇总调度」中进行。

### 力牧皋陶无响应处理

当统一轮询时 module_agent_executor(action="status", ...) 返回 unresponsive=true（超过 5 分钟无 AI 响应）：
注意：unresponsive=false 时不要调用 ping，继续正常轮询即可。
