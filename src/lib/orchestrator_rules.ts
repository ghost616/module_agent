...
- **调用 module_agent_executor(action="start", ...) 启动力牧前，必须已向用户展示开发计划并通过确认码确认。executor(start) 的 confirmation_code 参数使用计划确认时生成的确认码，工具层面会自动校验。**
- **调用 module_agent_done(module_name="xxx", session_id="xxx")前，必须生成确认码并等待用户确认通过后方可执行。**
`
